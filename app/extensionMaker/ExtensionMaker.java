...
public class ExtensionMaker {
    ...
    public String[][] buildStatements(SchemaDef schemaDef) {
        int columm = schemaDef.getTableDefList().size();
        int row = 20;
        String[][] insertStatements = new String[row][columm];
        ...
        for(int i = 0; i < insertStatements.length; i++) {
            for(int j = 0; j < columm; j++) {
                switch (tableDefs.get(0).getColumnDefList().get(j).getName()) {
                    case "mail":
                        for ( int b = 0; b <= j; b ++){
                            if( tableDefs.get(0).getColumnDefList().get(b).getName().equals("firstname") || tableDefs.get(0).getColumnDefList().get(b).getName().equals("lastname")){
                                if(insertStatements[i][j] == null ){
                                    insertStatements[i][j] = insertStatements[i][b];
                                }else{insertStatements[i][j] = insertStatements[i][j] + "." + insertStatements[i][b];}
                            }
                        }
                        insertStatements[i][j] = insertStatements[i][j] + (int) (Math.random() * 100) +"@"+ mail[(int)(Math.random() * mail.length)];
                        break;
                    case "firstname":
                        insertStatements[i][j] = "" + firstname[(int) (Math.random() * firstname.length)];
                        break;
                    case "animal":
                        insertStatements[i][j] = "" + animal[(int) (Math.random() * animal.length)];
                        break;
                    case "city":
                        insertStatements[i][j] = "" + city[(int) (Math.random() * city.length)];
                        break;
                    case "title":
                        insertStatements[i][j] = "" + title[(int) (Math.random() * title.length)];
                        break;
                    case "fullname":
                        insertStatements[i][j] = "" + firstname[(int) (Math.random() * firstname.length)] + " " + lastname[(int) (Math.random() * lastname.length)];
                        break;
                    case "metal":
                        insertStatements[i][j] = "" + metal[(int) (Math.random() * metal.length)];
                        break;
                    case "day":
                        insertStatements[i][j] = "" + (int) (Math.random() * 30);
                        break;
                    case "month":
                        insertStatements[i][j] = "" + (int) (Math.random() * 13);
                        break;
                    case "year":
                        insertStatements[i][j] = "" + (1930 + (int) (Math.random() * 87));
                        break;
                    case "word":
                        insertStatements[i][j] = word[(int) (Math.random() * word.length)] + " " + word[(int) (Math.random() * word.length)];
                        break;
                    case "lastname":
                        insertStatements[i][j] = "" + lastname[(int) (Math.random() * lastname.length)];
                        break;
                    case "colour":
                        insertStatements[i][j] = "" + colour[(int) (Math.random() * colour.length)];
                        break;
                    case "country":
                        insertStatements[i][j] = "" + country[(int) (Math.random() * country.length)];
                        break;
                    default:
                        insertStatements[i][j] = "" + tableDefs.get(0).getColumnDefList().get(j).getName();
                        break;
                }
            }
        }
